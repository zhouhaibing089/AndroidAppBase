...
public class TabBarView extends EqualLinearLayout
  ...
  private void init(AttributeSet attrs) {
    final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.AppBaseTabBarView);
  ...
  protected LayoutParams getTabLayoutParams() {
    EqualLinearLayout.LayoutParams params =
        new EqualLinearLayout.LayoutParams(EqualLinearLayout.LayoutParams.WRAP_CONTENT,
            EqualLinearLayout.LayoutParams.WRAP_CONTENT);
    params.setPercent(1.0f / mAdapter.getCount());
    return params;
  }
  ...
  @Override public void onPageScrolled(int position, float offset, int positionOffsetPixels) {
    if (offset == 0f) {
      changeTab(position);
      return;
    }
  ...
  @Override public void onClick(View v) {
    int to = indexOfChild(v);
    if (mPager != null) {
      int from = mPager.getCurrentItem();
      if (from == to) {
        return;
      }
      if (mTabChangeHook.onChange(from, to)) {
        mPager.setCurrentItem(to, false);
      }
    } else {
      /* 手动修改Tab */
      if (mTabChangeHook.onChange(mTabIndex, to)) {
        changeTab(to);
      }
    }
  }

  TabChangeHook mTabChangeHook = new DefaultTabChangeHook();

  public void setTabChangeHook(TabChangeHook hook) {
    mTabChangeHook = hook;
  }

  public interface TabChangeHook {
    boolean onChange(int from, int to);
  }

  public static class DefaultTabChangeHook implements TabChangeHook {
    @Override public boolean onChange(int from, int to) {
      return from != to;
    }
  }
  ...
  public void setBadge(int position, int count) {
    ...
    if (view instanceof FrameLayout) {
      ...
      FrameLayout.LayoutParams badgeParams =
          new FrameLayout.LayoutParams((int) AppUtils.getPix(getContext(), dp),
